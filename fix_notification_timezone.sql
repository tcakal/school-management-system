...
-- Redefine Function with CORRECT Timezone Logic
...
BEGIN
    -- Log Start
    INSERT INTO debug_notification_logs (message) VALUES ('Cron Job Started (Timezone Fix Applied)');

    -- FIXED TIMEZONE CALCULATION
    -- now() returns UTC. We want Istanbul Wall Clock Time as a TIMESTAMP.
    -- Simple conversion:
    ...
    FOR v_template IN SELECT * FROM notification_templates WHERE is_active = true AND trigger_type = 'fixed_time' LOOP
        ...
        IF v_template.trigger_time IS NOT NULL THEN
            ...
            -- Check if within 60 mins AFTER target time
