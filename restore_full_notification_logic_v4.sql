-- RESTORE FULL NOTIFICATION LOGIC V4
-- Supports:
-- 1. lesson_start, lesson_end, 15_min_before (Lesson Level)
-- 2. last_lesson_end (Day/School Level)
-- 3. fixed_time (Fixed Clock Level)
    ...
    FOR v_template IN SELECT * FROM public.notification_templates WHERE is_active = true LOOP
        ...
        IF v_template.trigger_type = 'fixed_time' THEN
            IF v_template.trigger_time = ANY(v_recent_times) THEN
                PERFORM 1 FROM notification_logs WHERE template_id = v_template.id AND sent_at::date = v_now::date;
                IF NOT FOUND THEN
                    ...
                    IF v_template.target_roles::jsonb ? 'admin' THEN
                        IF v_settings.admin_chat_id IS NOT NULL THEN INSERT INTO temp_recipients (chat_id, role) VALUES (v_settings.admin_chat_id, 'admin'); END IF;
                    END IF;
                    ...
                    v_message := replace(replace(v_template.message_template, '{class_name}', 'Genel'), '{start_time}', v_template.trigger_time);
                    FOR v_recipient IN SELECT DISTINCT chat_id, role FROM temp_recipients LOOP
                        INSERT INTO notification_logs (template_id, recipient_chat_id, recipient_role, message_body) VALUES (v_template.id, v_recipient.chat_id, v_recipient.role, v_message);
                        PERFORM net.http_post(url := 'https://api.telegram.org/bot' || v_settings.telegram_bot_token || '/sendMessage', body := json_build_object('chat_id', v_recipient.chat_id, 'text', v_message)::jsonb);
                    END LOOP;
                END IF;
            END IF;

        -- C. Handle Lesson-Based (lesson_start, lesson_end, 15_min_before, last_lesson_end)
        ELSIF v_template.trigger_type IN ('lesson_start', 'lesson_end', '15_min_before', 'last_lesson_end') THEN
            FOR v_lesson IN 
                ...
                AND (v_template.trigger_type != 'last_lesson_end' OR l.end_time = (SELECT MAX(end_time) FROM lessons WHERE date = l.date AND school_id = l.school_id AND status != 'cancelled'))
            LOOP
                BEGIN
                    DELETE FROM temp_recipients;
