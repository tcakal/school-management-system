-- RESTORE PRODUCTION LOGIC (With Trace Logging)
-- This script enables the REAL sending logic with all fixes applied.
-- Fixes included:
-- 1. JSON Syntax: '["admin"]' instead of '{"admin"}'
-- 2. Student Support: Added loop for students.
-- 3. Trace Logging: Keeps logging so we can verify success.
...
BEGIN
    ...
    FOR v_template IN SELECT * FROM public.notification_templates WHERE is_active = true LOOP
        ...
        IF v_template.trigger_type = 'fixed_time' THEN
            -- Check (Verbose)
            INSERT INTO public.debug_trace_logs (message) VALUES ('CHECK TEMPLATE: ' || v_template.trigger_time || ' vs ' || v_current_time_str);

            IF v_template.trigger_time = v_current_time_str THEN
                INSERT INTO public.debug_trace_logs (message) VALUES ('MATCH FOUND: Template ' || v_template.id);
