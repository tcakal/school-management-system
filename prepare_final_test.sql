...
TRUNCATE TABLE public.debug_trace_logs;

-- 2. Re-apply Function with Verbose Logging (Force Update)
...
BEGIN
    ...
    FOR v_template IN SELECT * FROM public.notification_templates WHERE is_active = true LOOP
        ...
        IF v_template.trigger_type = 'fixed_time' THEN
            -- TRACE: Check Template (VERBOSE MODE ON)
            ...
            IF v_template.trigger_time = v_current_time_str THEN
                INSERT INTO public.debug_trace_logs (message) VALUES ('MATCH FOUND: Template ' || v_template.id || ' Time: ' || v_template.trigger_time);
                ...
                ELSE
                    ...
                    -- 3. MANAGERS (Simplified for trace)
                    ...
                    END IF;
                     ...
                     -- 4. STUDENTS (Simplified for trace)
