...
BEGIN
    -- Log Start
    INSERT INTO debug_notification_logs (message) VALUES ('Cron Job Started (Verbose + Fix)');
    ...
    FOR v_template IN SELECT * FROM notification_templates WHERE is_active = true AND trigger_type IN ('lesson_start', 'lesson_end') LOOP
        ...
        -- Logging
        INSERT INTO debug_notification_logs (message, details) 
        VALUES ('Checking Template', jsonb_build_object('id', v_template.id, 'type', v_template.trigger_type));

        -- Day Filter
        IF v_template.days_filter IS NOT NULL AND NOT (v_template.days_filter::jsonb @> to_jsonb(v_day_of_week)) THEN
            INSERT INTO debug_notification_logs (message) VALUES ('Skipped: Day Filter');
            CONTINUE;
        END IF;
        ...
        LOOP
            BEGIN
                ...
                IF v_diff_minutes >= 0 AND v_diff_minutes <= 60 THEN
                    ...
                    ELSE
                        INSERT INTO debug_notification_logs (message, details) VALUES ('Skipped: Already Sent', jsonb_build_object('id', v_identifier));
                    END IF;
                ELSE
