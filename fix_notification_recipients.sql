...
BEGIN
    -- Log Start
    INSERT INTO debug_notification_logs (message) VALUES ('Cron Job Started (Recipient Fix)');
    ...
    -- Create Temp Table for Deduplication (Auto-drops at end of transaction)
    ...
    FOR v_template IN SELECT * FROM notification_templates WHERE is_active = true AND trigger_type IN ('lesson_start', 'lesson_end') LOOP
        ...
        LOOP
            BEGIN
                -- Clear Temp Table for this specific notification
                ...
                -- Check Time Window (0 to 60 minutes passed)
                IF v_diff_minutes >= 0 AND v_diff_minutes <= 60 THEN
                    ...
                    IF NOT EXISTS (SELECT 1 FROM notification_logs WHERE notification_identifier = v_identifier) THEN
                        ...
                        -- Manager (School Owner)
                        IF v_template.target_roles::jsonb ? 'manager' AND v_lesson.school_id IS NOT NULL THEN
                             -- Explicitly fetch manager (who is a user with role 'manager' and id = school_id, effectively)
                             -- OR if your schema uses telegram_chat_id in schools table:
                             INSERT INTO temp_recipients (chat_id)
            ...
            EXCEPTION WHEN OTHERS THEN
                INSERT INTO debug_notification_logs (message, details) VALUES ('Lesson Calc Error', jsonb_build_object('err', SQLERRM));
            END;
        END LOOP;
    END LOOP;
        ...
        IF v_template.trigger_time IS NOT NULL THEN
            ...
            IF v_diff_minutes >= 0 AND v_diff_minutes <= 60 THEN
                ...
                IF NOT EXISTS (SELECT 1 FROM notification_logs WHERE notification_identifier = v_identifier) THEN
                    ...
                    -- 1. Collect Recipient IDs (Since fixed time is SCHOOL WIDE vs CLASS WIDE)
                    -- Note: Fixed Time templates usually have a school_id
                    
                    -- Student / Parent (All students in that school)
                    IF v_template.target_roles::jsonb ? 'student' THEN
                        INSERT INTO temp_recipients (chat_id)
                    ...
                    -- Teacher (All teachers in that school)
