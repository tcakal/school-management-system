-- Add Branch Manager Notification Support
-- This updates the check_and_send_notifications function to handle 'branch_manager' role.
...
DECLARE
    ...
    v_school record;
    v_student record;
    v_branch_manager record; -- NEW: For Branch Managers
    i int;
BEGIN
        ...
        IF v_template.trigger_type = 'fixed_time' THEN
            ...
            IF v_template.trigger_time = ANY(v_recent_times) THEN
                ...
                ELSE
                    ...
                    -- 3. SCHOOL MANAGERS (Okul Müdürleri)
                    IF v_template.target_roles @> '["manager"]' THEN
                        INSERT INTO public.debug_trace_logs (message) VALUES ('SENDING: School Managers');
                    ...
                    -- 4. BRANCH MANAGERS (Şube Yöneticileri) -- NEW LOGIC
                    IF v_template.target_roles @> '["branch_manager"]' THEN
                        INSERT INTO public.debug_trace_logs (message) VALUES ('SENDING: Branch Managers');
                        -- Query branches, join with teachers (managers) to get telegram_chat_id
                        FOR v_branch_manager IN 
                            SELECT t.telegram_chat_id, b.name as branch_name 
                            FROM public.branches b 
                            JOIN public.teachers t ON b.manager_id = t.id 
                            WHERE t.telegram_chat_id IS NOT NULL
                        LOOP
                            INSERT INTO public.notification_logs (template_id, target_id, recipient_chat_id, recipient_role, message_body) VALUES (v_template.id, NULL, v_branch_manager.telegram_chat_id, 'branch_manager', v_message);
                            PERFORM net.http_post(url := 'https://api.telegram.org/bot' || v_bot_token || '/sendMessage', body := jsonb_build_object('chat_id', v_branch_manager.telegram_chat_id, 'text', v_message));
                        END LOOP;
                    END IF;
