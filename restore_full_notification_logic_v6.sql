-- RESTORE FULL NOTIFICATION LOGIC V6 (MULTI-TEACHER SUPPORT)
-- 1. Support teacher_ids UUID array for multiple teacher notifications.
-- 2. Strict idempotency maintained via notification_identifier.
...
BEGIN
    ...
    IF v_bot_token IS NULL OR v_bot_token = '' THEN RETURN; END IF;
                ...
                BEGIN
                    ...
                    IF v_diff_minutes >= 0 AND v_diff_minutes <= 60 THEN
                        ...
                        IF NOT EXISTS (SELECT 1 FROM public.notification_logs WHERE notification_identifier = v_identifier) THEN
                            ...
                            -- Target teachers in teacher_ids array
